...
# Save provider snapshot (overwrites each run)
...

{
  ...
  if [ "$UPDATED" -gt 0 ]; then
    echo "RESULT: $UPDATED titles updated"
  else
    echo "RESULT: No title changes"
  fi
  echo "──────────────────────────────────────────"
  echo
  echo
...
EOF
  ...
  KEEP=20
  if [ "$TOTAL" -gt "$KEEP" ] && [ "$TOTAL" -gt 0 ]; then
